...
shinyServer(function(input, output) {
  output$parentsText <- renderText({
    paste("When the father's height is",  strong(round(input$inFh, 1)),"cm, and mother's is",
  ...
  output$prediction <- renderText({
    ...
    sord <- ifelse(  input$inGen=="female","Daugther", "Son" )
    paste0(em(strong(sord)), "'s predicted height would be approximately ", em(strong(round(ch)))," cm"
    )
  })
